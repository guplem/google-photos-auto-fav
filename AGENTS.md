...
- **Google Photos ignores an arrow key whose target is a button.** The user starts a run by clicking the panel's Start button, so that button keeps the focus and every synthetic `ArrowRight` went to it and did nothing: the run stopped on photo one with `stuck`. `takeFocusOffButtons` blurs any focused button or `role="button"` before a key is sent. The arrow key is the main way to advance and **every** attempt sends it; the named next control is only a backup on odd attempts.
- **The info panel pushes the next-photo chevron away from the right edge.** The panel takes the right third of the window, and this extension keeps it open for the whole run. `RIGHT_EDGE_FRACTION` is therefore `0.5`, not `0.8`, and `findNextControl` takes the right-most match instead of the first. The name match is the real guard; the fraction only breaks a tie.
- **The info panel is a toggle.** `requestInfoPanel(attempt)` presses `i` on even attempts and clicks the named control on odd ones. Never both in one call: an earlier version sent the key and then clicked, so the key opened the panel and the click closed it every time.
